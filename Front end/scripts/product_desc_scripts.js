...
if (localStorage.getItem("SmartStockers_cart") == null) {
    localStorage.setItem("SmartStockers_cart", JSON.stringify([]));
}
...
function addToCart(){
    let cartArr = JSON.parse(localStorage.getItem("SmartStockers_cart")); 
    if(sipInput.value == "" || sipInput.value < 100 || sipInput.value > 50000){
        ...
        if(flag){
            ...
            localStorage.setItem("SmartStockers_cart", JSON.stringify(cartArr));
            console.log(cartArr);
...
function goToCart(){
    window.location.href = "SmartStockers_cart.html";
}
